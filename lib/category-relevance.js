...
var ReviewRankRelevance = (function () {
  ...
  // Accepts an optional adapter to support site-specific query params:
  //   Amazon: k=
  //   Flipkart: q=
  //   Meesho: q=
  //   Myntra: q= or search=
  // Returns '' when absent. Never throws.
  function extractSearchQueryFromUrl(url, adapter) {
    ...
    try {
      ...
      var u = new URL(base);
      var paramNames = ['k', 'q', 'search'];
      if (adapter && adapter.name) {
        if (adapter.name === 'Amazon') paramNames = ['k'];
        else if (adapter.name === 'Flipkart') paramNames = ['q'];
        else if (adapter.name === 'Meesho') paramNames = ['q'];
        else if (adapter.name === 'Myntra') paramNames = ['q', 'search'];
      }
      for (var i = 0; i < paramNames.length; i++) {
        var val = u.searchParams ? u.searchParams.get(paramNames[i]) : null;
        if (val && val.trim()) return val.replace(/\+/g, ' ').trim();
      }
