...
function parseAmazonProduct(item) {
  ...
  var rawRatingCount = extractReviewCount(item);
...
function parseFlipkartProduct(card) {
  ...
  var ratingCount = extractFlipkartRatingCount(card);
...
function extractFlipkartRatingCount(card) {
  // Strategy 1: specific class for ratings count
  var selectors = [
    'span._2U9tEA',
    '._2U9tEA',
  ...
  for (var s = 0; s < selectors.length; s++) {
    ...
    for (var e = 0; e < els.length; e++) {
      var text = (els[e].textContent || '').trim();
      var count = parseFlipkartReviewText(text, 'rating');
      if (count !== null && count > 0) return count;
    }
  }

  // Strategy 2: scan all spans for "X Ratings" pattern
  var spans = card.querySelectorAll('span');
  for (var i = 0; i < spans.length; i++) {
    var text = (spans[i].textContent || '').trim();
    if (!text || text.length > 60) continue;
    var lower = text.toLowerCase();
    // Only look for rating patterns
    if (lower.indexOf('rating') === -1) continue;
...
function extractFlipkartReviewCount(card) {
  // Strategy 1: specific class for reviews count, plus _2U9tEA (Flipkart
  // shows rating count here; used as review count when no separate Reviews text)
  var selectors = [
  ...
  // Strategy 2: scan all spans for "X Reviews" pattern
  var spans = card.querySelectorAll('span');
  for (var i = 0; i < spans.length; i++) {
    var text = (spans[i].textContent || '').trim();
    if (!text || text.length > 60) continue;
    var lower = text.toLowerCase();
    // Only look for review patterns, not rating patterns
    if (lower.indexOf('review') === -1) continue;
    if (lower.indexOf('rating') !== -1) continue;
    if (/[₹$€]/.test(text)) continue;
    if (/%/.test(text)) continue;
    var count = parseFlipkartReviewText(text, 'review');
    if (count !== null && count > 0 && count < 100000000) {
      return count;
    }
  }
...
function parseFlipkartReviewText(text, type) {
  if (!text) return null;
  var lower = text.toLowerCase();
  var wantedType = type || 'rating';

  // Pattern: "64,627 Ratings & 4,820 Reviews"
  // Extract only the count matching the requested type
  var ratingsMatch = lower.match(/([\d.,]+)\s*(k|l)?\s*ratings?/);
  if (ratingsMatch && wantedType === 'rating') {
    var num = parseInt(ratingsMatch[1].replace(/,/g, ''), 10);
    if (!isNaN(num) && num > 0) return num;
  }

  var reviewsMatch = lower.match(/([\d.,]+)\s*(k|l)?\s*reviews?/);
  if (reviewsMatch && wantedType === 'review') {
    var num = parseInt(reviewsMatch[1].replace(/,/g, ''), 10);
    if (!isNaN(num) && num > 0) return num;
  }

  // "(2,345)" — parenthesized count, only if text mentions the wanted type
  if (lower.indexOf(wantedType) !== -1) {
  ...
  // Generic parenthesized count fallback (e.g., "(1,234)" without type keyword)
  var genericParen = text.match(/\(([\d.,]+)\)/);
  if (genericParen) {
    var gnum = parseInt(genericParen[1].replace(/,/g, ''), 10);
...
function parseMeeshoProduct(card) {
  ...
  var ratingCount = extractMeeshoRatingCount(card);
...
function extractMeeshoRatingCount(card) {
  var selectors = [
    '[class*="Ratings"]',
    '[class*="ratings"]',
    'span[class*="Rating"]',
  ...
  for (var s = 0; s < selectors.length; s++) {
    ...
    for (var e = 0; e < els.length; e++) {
      var text = (els[e].textContent || '').trim();
      var count = parseMeeshoReviewText(text, 'rating', true);
      if (count !== null && count > 0) return count;
    }
  }
  // Fallback: scan spans that mention "rating" in their text.
  var spans = card.querySelectorAll('span');
  for (var i = 0; i < spans.length; i++) {
    var text = (spans[i].textContent || '').trim();
    if (!text || text.length > 60) continue;
    ...
    if (/[₹$€]/.test(text)) continue;
    if (/%/.test(text)) continue;
    var count = parseMeeshoReviewText(text, 'rating', false);
    if (count !== null && count > 0 && count < 100000000) {
      return count;
    }
  }
...
function extractMeeshoReviewCount(card) {
  ...
  for (var s = 0; s < selectors.length; s++) {
    ...
    for (var e = 0; e < els.length; e++) {
      var text = (els[e].textContent || '').trim();
      var count = parseMeeshoReviewText(text, 'review', false);
      if (count !== null && count > 0) return count;
    }
  }
  // Fallback: scan spans that mention "review" in their text,
  // but NOT "rating" (which would be extracted by extractMeeshoRatingCount).
  var spans = card.querySelectorAll('span');
  for (var i = 0; i < spans.length; i++) {
    var text = (spans[i].textContent || '').trim();
    if (!text || text.length > 60) continue;
...
function parseMeeshoReviewText(text, type, ratingContext) {
  if (!text) return null;
  var lower = text.toLowerCase();

  // Helper: parse a number string that may include commas and optional
  // k/K (thousands) or l/L (lakhs) suffix, e.g. "1,299", "12.4K", "1.2L".
  function parseScaled(numStr, suffix) {
    var num = parseFloat(numStr);
    if (!isNaN(num) && num > 0) {
      if (suffix === 'k') num *= 1000;
      else if (suffix === 'l') num *= 100000;
  ...
  // "1,299 Ratings", "1299 Ratings", "12.4K Ratings", "1.2L Ratings"
  var ratingsMatch = lower.match(/([\d.,]+)\s*(k|l)?\s*ratings?/);
  if (ratingsMatch && wantedType === 'rating') {
    var num = parseScaled(ratingsMatch[1].replace(/,/g, ''), ratingsMatch[2] ? ratingsMatch[2].toLowerCase() : '');
    if (num !== null) return num;
  }

  // "493 Reviews", "12.5K Reviews"
  var reviewsMatch = lower.match(/([\d.,]+)\s*(k|l)?\s*reviews?/);
  if (reviewsMatch && wantedType === 'review') {
    var num = parseScaled(reviewsMatch[1].replace(/,/g, ''), reviewsMatch[2] ? reviewsMatch[2].toLowerCase() : '');
    if (num !== null) return num;
  }

  // "(1,299)" — parenthesized count.
  // When ratingContext is true (class-based selector), accept bare parentheses.
  // When false (fallback span scan), require the type word in the text
  // to avoid picking up unrelated parenthesized numbers.
  if (ratingContext || lower.indexOf(wantedType) !== -1) {
    var parenMatch = text.match(/\(([\d.,]+)\)/);
    if (parenMatch) {
      var num = parseScaled(parenMatch[1].replace(/,/g, ''), '');
      if (num !== null) return num;
    }
  }
