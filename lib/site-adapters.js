...
function extractMeeshoReviewCount(card) {
  ...
  for (var s = 0; s < selectors.length; s++) {
    ...
    for (var e = 0; e < els.length; e++) {
      var text = (els[e].textContent || '').trim();
      // Class-based selectors already provide rating/review context.
      var count = parseMeeshoReviewText(text, true);
      if (count !== null && count > 0) return count;
    }
  }
  // Fallback: scan spans that contain "rating" or "review" in their text.
  // This avoids picking up unrelated numbers (prices, offer counts, etc.)
  // that happen to be bare numbers in other spans.
  var spans = card.querySelectorAll('span');
  for (var i = 0; i < spans.length; i++) {
    var text = (spans[i].textContent || '').trim();
    if (!text || text.length > 60) continue;
    var lower = text.toLowerCase();
    // Only accept spans whose text mentions ratings/reviews.
    if (lower.indexOf('rating') === -1 && lower.indexOf('review') === -1) continue;
    if (/[₹$€]/.test(text)) continue;
    if (/%/.test(text)) continue;
    var count = parseMeeshoReviewText(text, false);
    if (count !== null && count > 0 && count < 100000000) {
      return count;
    }
  }
...
function parseMeeshoReviewText(text, ratingContext) {
  if (!text) return null;
  var lower = text.toLowerCase();
  ...
  if (ratingsMatch) {
    var num = parseScaled(ratingsMatch[1], ratingsMatch[2] ? ratingsMatch[2].toLowerCase() : '');
  ...
  if (reviewsMatch) {
    var num = parseScaled(reviewsMatch[1], reviewsMatch[2] ? reviewsMatch[2].toLowerCase() : '');
    if (num !== null) return num;
  }

  // "(1,299)" — parenthesized count.
  // When ratingContext is true (class-based selector), accept bare parentheses.
  // When false (fallback span scan), require "rating"/"review" in the text
  // to avoid picking up unrelated parenthesized numbers.
  if (ratingContext || lower.indexOf('rating') !== -1 || lower.indexOf('review') !== -1) {
    var parenMatch = text.match(/\(([\d.,]+)\)/);
    if (parenMatch) {
      var num = parseScaled(parenMatch[1], '');
      if (num !== null) return num;
    }
  }
